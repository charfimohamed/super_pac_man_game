...
public class Sprite extends ImageGraphics {
	...
	/**
	 * Extracts from an image the sprites corresponding to a given orientation the
	 * returned array has 4 entry (one per orientation) the content of each entry is
	 * an array of sprites corresponding to the given orientation (the entry indexed
	 * by Orientation.dir.ordinal() is the array of sprites corresponding to the
	 ...
	 */
	public static Sprite[][] extractSprites(String name, int nbFrames, float width, float height, Positionable parent,
			int regionWidth, int regionHeight, Vector anchor, Orientation[] order) {
		...
		for (int i = 0; i < nbFrames; i++) {
			...
			sprites[order[0].ordinal()][i] = new Sprite(name, width, height, parent,
					new RegionOfInterest(i * regionWidth, regionHeight * j++, regionWidth, regionHeight), anchor);
	 ...
	 */
	public static Sprite[][] extractSprites(String name, int nbFrames, float width, float height, Positionable parent,
			int regionWidth, int regionHeight, Orientation[] order) {
	 ...
	 */
	public static Sprite[] extractSprites(String name, int nbFrames, float width, float height, Positionable parent,
			Vector anchor, int regionWidth, int regionHeight) {
		Sprite[] sprites = new Sprite[nbFrames];

		for (int i = 0; i < nbFrames; i++) {
			sprites[i] = new Sprite(name, width, height, parent,
					new RegionOfInterest(i * regionWidth, 0, regionWidth, regionHeight), anchor);
		}
